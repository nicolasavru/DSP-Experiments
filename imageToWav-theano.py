...
def oscillator(x, freq=1, amp=1, base=0, phase=0):
    f = function([],
                 sandbox.cuda.basic_ops.gpu_from_host(base + amp * tensor.sin(2 * np.pi * freq * x + phase)))
    o = f()
    print o
    return o
...
for x in range(xres):
    ...
    t = np.arange(x*xlen, x*xlen + xlen, 1./44100, dtype=np.float32)
    t_gpu = shared(t)
    tone_gpu = shared(np.zeros(t.size, dtype=np.float32))
...
#        amplitude = math.log(amplitude+1)# / math.log(255)
        ...
        if p[r] > 10 or p[g] > 10 and p[b] > 10:
            a = oscillator(t_gpu,
                           ...
                           freq=yscale * (yres - y))
            print "a", a
            tone_gpu = tensor.add(tone_gpu, a)
            print "b", tone_gpu
    tone_gpu = tone_gpu + 1
